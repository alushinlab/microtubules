...
# modified Alushin lab, alushing@mail.nih.gov
...
def processHelicalVol(vol,voleve,volodd,iref,outdir,itout,dp,dphi,apix,hsearch,findseam=False,vertstep=None,wcmask=None):
	...
	dp,dphi = hsearchsym(vol,dp,dphi,apix,hsearch[1],hsearch[0],0.03)
	...
	if findseam is True:
		...
		#helicise now requires a fractional region of the volume to be specified, 2/3 should be fine for most cases
		helical_region = 0.67
		helical_region = float(helical_region)
		#also, in new version of SPARX it appears that sign of twist has been reversed, change it for symmetrization
		sparxdphi=-1*dphi
		vol = vol.helicise(apix, dp, sparxdphi, helical_region)
		voleve = voleve.helicise(apix, dp, sparxdphi, helical_region)
		volodd = volodd.helicise(apix, dp, sparxdphi, helical_region)
...
def hsearchsym(vol,dp,dphi,apix,rmax,rmin,hstep):
	...
	emancmd = "/programs/x86_64-linux/eman/1.9/bin/proc3d %s %s spidersingle"%(volrot,volrot)
